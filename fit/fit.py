...
class Template(object):
    ...
    def buildSplineTemplate(self,frames,args):
        """
...
class FrameProcessor(object):
    def __init__(self,tabs,args,db):
        ...
        if self.args.load_template is not None:
            ...
            try:
                self.template = Template(templateData,self.mostRecentTemplateTimestamp)
            except IndexError:
                self.template = None
                # Output zeroes
        else:
    ...
    def updateTemplate(self):
        ...
        if self.args.load_template == "db":
            ...
            if templateTimestamp != self.mostRecentTemplateTimestamp:
                self.template = Template(templateData,templateTimestamp)
