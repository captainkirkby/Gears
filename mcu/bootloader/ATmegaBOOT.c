...
char gethex(void);
void puthex(char);
void flash_led(uint8_t, uint8_t);
...
int main(void)
{
    ...
    /* set LED pins to HIGH (PLATFORM SPECIFIC!!)*/
    led_off(RED);
    led_off(YELLOW);
    led_off(GREEN);
	...
	flash_led(NUM_LED_FLASHES, YELLOW);

	#ifdef	ADABOOT
		flash_led(ADABOOT_VER, YELLOW);		// BBR 9/13/2008
	#endif 
...
void flash_led(uint8_t count, uint8_t led)
{
